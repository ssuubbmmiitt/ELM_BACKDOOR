...
def train_someBatch(model, optimizer, train_loader, batchidx=0):
    init = time.time()
    model.train()
    correct = 0
    for batch_idx, (data, target) in enumerate(train_loader):
...
def train_sequential(model, train_loader, optimizer, starting_batch_idex=0):
    model.train()
    correct = 0
    for batch_idx, (data, target) in enumerate(train_loader):
        if batch_idx >= starting_batch_idex:
            ...
            print('\n{}st Batch train set accuracy: {}/{} ({:.2f}%)\n'.format(batch_idx,
                        ...
                        batch_idx + 1 - starting_batch_idex)),
                                                                              100. * correct / (
